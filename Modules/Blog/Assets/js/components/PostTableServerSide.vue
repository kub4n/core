    ...
    <div class="div">
        <div class="content-header">
            ...
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item :to="{name: 'admin.blog.post.index'}">{{ trans('posts.posts') }}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
                        ...
                        <div class="sc-table">
                            <div class="tool-bar el-row" style="padding-bottom: 20px;">
                                <div class="actions el-col el-col-8">
                                    ...
                                    <router-link :to="{name: 'admin.blog.post.create'}">
                            ...
                            <el-table
                                ...
                                <el-table-column prop="translations.title" :label="trans('posts.title')">
                                    <template slot-scope="scope">
                                ...
                                <el-table-column prop="translations.slug" :label="trans('posts.slug')">
                                    <template slot-scope="scope">
                                        <a @click.prevent="goToEdit(scope)" href="#">
                                ...
                                <el-table-column prop="actions" :label="trans('core.table.actions')">
                                    <template slot-scope="scope">
                                        <el-button-group>
                                            <edit-button :to="{name: 'admin.blog.post.edit', params: {postId: scope.row.id}}"></edit-button>
        ...
        <button v-shortkey="['c']" @shortkey="pushRoute({name: 'admin.blog.post.create'})" v-show="false"></button>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            queryServer(customProperties) {
                ...
                axios.get(route('api.blog.post.indexServerSide', _.merge(properties, customProperties)))
                    .then((response) => {
            ...
            handleExtraActions(action) {
                const postIds = _.map(this.selectedPosts, elem => elem.id);
                axios.get(route('api.blog.post.mark-status', { action, postIds: JSON.stringify(postIds) }))
                    .then((response) => {
                        this.$message({
                            type: 'success',
                        ...
                        this.data.filter(post => postIds.indexOf(post.id) >= 0)
                            .map((p) => {
                                const post = p;
                                post.translations.status = action === 'mark-online';
                                return post;
                            });
                    })
                    .catch(() => {
            ...
            handleSelectionChange(selectedPosts) {
